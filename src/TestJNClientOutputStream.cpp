...
#include "TestJNClientOutputStream.h"

using namespace std;

namespace JournalServiceServer
{
  TestJNClientOutputStream::TestJNClientOutputStream(
      const long txId,
	  const string data)
	  : mSize(0),
	    mOpcode(OPCODE),
  ...
  template <typename T>
    void
    TestJNClientOutputStream::store_as_big_endian(T u)
    {
      union
      {
        T u;
  ...
  void
    TestJNClientOutputStream::writeChecksum()
    {
  ...
  const char*
    TestJNClientOutputStream::get(unsigned int& length)
  {
    length = mSize;
    return mBuffer;
  }
